...
TRAFFIC_CONTROLLER::TRAFFIC_CONTROLLER(string crossid, MAP& map1,string file)
{
	time = 0;
	start_index = 0;
	terminate_num = 0;
...
bool TRAFFIC_CONTROLLER::IS_TRAN_MOVING_FINISHED()
{
	for (size_type i = 0; i < tarc_id_sequence.size(); i++)
	{
		tarc_id_sequence[i]->cur_road->ROAD_ARC_UPDATE_CHECK();
...
void TRAFFIC_CONTROLLER::HANDLE_TERMINATE_CAR(MOVING_CAR_PTR terminate_car)
{
	...
	std::cout << std::endl;
	ANSWER_WRITE write_car(filename);
	write_car.WRITE_MOVING_CAR_ITEMS(terminate_car);
	return;
}
