...
enum 
{
	...
	PAR_MAC_G43_TRANS_PROG_ID,       /*!< G43转换程序号 */
	PAR_MAC_G49_TRANS_PROG_ID,       /*!< G49转换程序号 */
	PAR_MAC_M00_TRANS_PROG_ID,
	PAR_MAC_MAX_REVERSE_INC_FIX_LEN, /*!< 反向增量长度过滤最大值 */
...
enum 
{
	...
	PAR_CH_TOOL_OFF_TYPE,       /*!<是否以坐标偏移方式执行刀具偏置 */
	...
	PAR_CH_FREQTAP_SPDL_ACCE_COEF,  /*!<变频主轴刚性攻丝主轴加速公式系数 */
 ...
 * @{
 */
#define SERVO_PARM_START_IDX 500 /*! 伺服参数起始参数号 */
#define SERVO_PARM_TOTAL_NUM 500 /*! 伺服参数个数 */
enum 
{
...
#define AX_NC_CYC64_MODE 0x1000
	...
	PAR_SV_MOTOR_TYPE = (SERVO_PARM_START_IDX+43), 	/*!< 驱动器电机类型代码 */
	PAR_SV_MOTOR_RATING_CUR = (SERVO_PARM_START_IDX+86),	/*!< 电机额定电流 */
	
	PAR_ECAT_MOTOR_RATING_CUR = (SERVO_PARM_START_IDX+334),		/*!<EtherCat设备轴电机额定电流 */
	// 伺服参数（主轴，预留500个）
	PAR_SP_POSITION_GAIN = SERVO_PARM_START_IDX,	/*!< 位置控制位置比例增益 */
	PAR_SP_MOTOR_RATING_CUR = (SERVO_PARM_START_IDX+53),	/*!< IM电机额定电流 */
	PAR_SP_MOTOR_TYPE = (SERVO_PARM_START_IDX+59),		/*!< 驱动器电机类型代码 */
	/*!@}*/
...
/*! 数控设备控制对象应用类型定义，\n
	系统根据其类型和接口参数确定控制对象在系统中的输入输出数据地址 */
enum DevNcobjType
{
...
enum ToolParaIndex
{
	 ...
	 * @{
	 */
	WTOOL_LEN1 = MAX_GEO_PARA, /*!< (铣：长度磨损z；车：Z磨损) */
	 ...
	 * @{
	 */
    MOTOOL_TYPE = MAX_GEO_PARA+MAX_WEAR_PARA+MAX_TECH_PARA+MAX_TOOL_EXPARA, /*!< 刀具监控类型，0:关闭 1:安装次数 2:切削时间 3:磨损 4:切削里程 6:切削能耗 6:主轴转数 */
	MOTOOL_SEQU,		/*!< 	优先级 */
	MOTOOL_MULTI,		/*!< 	倍率 */
	...
	MOTOOL_MAX_COUNT,	/*!< 未使用 */
	MOTOOL_ALM_COUNT,	/*!< 未使用 */
	MOTOOL_ACT_COUNT,	/*!< 未使用 */

	MOTOOL_POWER_MAX,	/*!< 负载最大值 */
	MOTOOL_POWER_MIN,	/*!< 负载最小值 */
	MOTOOL_BREAK_CNT,	/*!< 断刀检测计数 */

	MOTOOL_GROUP,       /*!< 刀具所属分组号 */

	MOTOOL_ACT_LIFE_LO,	/*!< 实际寿命(低位) */
	MOTOOL_CNT_UNIT,	/*!< 计数单位(仅主轴转数用) 0:默认 1:千 2:万 3:十万 4:百万 5:千万 6:亿 */
	/*!@}*/
...
#define LTOOL_MEAS_X	METOOL_PARA0	/*!< 刀具测量X长度 */
#define LTOOL_MEAS_Y	METOOL_PARA1	/*!< 刀具测量Y长度 */
#define LTOOL_MEAS_Z	METOOL_PARA2	/*!< 刀具测量Z长度 */
/*!@}*/
/*!@}*/

#define SPDL_RESOLUTION	(1000)	/*!< 主轴转速分辨率 */
#define MAX_ROW_IN_ONE_SCAN 200
 ...
 * @{
 */
#define SMPL_MAX_CHAN_NUM (64)	/*!< 最大采样通道数。PS：内存上预留64个通道，但目前仅可用32个 */
#define SMPL_CHAN_NUM (32)		/*!< 可用采样通道数 */
...
/*!@}*/ 
#define SAMPL_TYPE_VALID(type)	((type > SAMPL_NULL_TYPE && type <= SAMPL_TOL_COMP) || (type >= SAMPL_SYS_VAL && type < SAMPL_TOTAL))
